...
def load_pretrained_weights(
    custom_model, model_name="paraphrase-MiniLM-L3-v2"
):
    """
    ...
    Args:
        ...
        model_name: Model name without organization prefix
    ...
    if config.hidden_size != custom_model.model.embeddings.word_embeddings.embedding_dim:
        raise ValueError(f"Hidden size mismatch: pretrained={config.hidden_size}, custom={custom_model.model.embeddings.word_embeddings.embedding_dim}")
    if config.num_hidden_layers != num_layers:
        raise ValueError(f"Layer count mismatch: pretrained={config.num_hidden_layers}, custom={num_layers}")
    if config.num_attention_heads != custom_model.model.encoder.layer[0].attention.num_attention_heads:
        raise ValueError(f"Attention head count mismatch: pretrained={config.num_attention_heads}, custom={custom_model.model.encoder.layer[0].attention.num_attention_heads}")
    
    # Load pretrained model
    pretrained_model = AutoModel.from_pretrained(local_model_path, local_files_only=True)
    ...
    for pretrained_name, custom_name in mapping.items():
        if pretrained_name in pretrained_dict and custom_name in custom_dict:
            if pretrained_dict[pretrained_name].shape != custom_dict[custom_name].shape:
                print(f"Shape mismatch for {pretrained_name}: pretrained={pretrained_dict[pretrained_name].shape}, custom={custom_dict[custom_name].shape}")
            else:
